defmodule YodelStudioWeb.UserAuthTest do
  use YodelStudioWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias YodelStudio.Accounts
  alias YodelStudio.Accounts.Scope
  alias YodelStudioWeb.UserAuth

  import YodelStudio.AccountsFixtures
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, YodelStudioWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      YodelStudioWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        YodelStudioWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
